...
def mlp(x=tf.placeholder(tf.float32, [None, 784]), hidden_sizes= [64, 10], activation_fn= tf.nn.relu):
  ...
  for i in range(len(io_sizes)-1):
    with tf.variable_scope("layer"+str(i)):
            weights= tf.get_variable("weights", [io_sizes[i], io_sizes[i+1]], initializer=tf.random_normal_initializer(stddev= 0.01))
            biases = tf.get_variable("biases", [io_sizes[i+1]], initializer=tf.constant_initializer(0.01))
...
def cosine_similarity(arr1, arr2):
    #logits = arr1 *  arr2 / (tf.norm(arr1, 2, 1) * tf.norm(arr2, 2, 1)) 
    logits= tf.nn.l2_normalize(arr2, 1, epsilon=1e-12, name="ele1") * tf.nn.l2_normalize(arr1, 1, epsilon=1e-12, name="ele2")
    #y_logits = tf.reduce_sum(logits, 1)
    return logits
    ###################################
    ####     PUT YOUR CODE HERE    ####
    ###################################
...
def build_model(mlp_args):
    with tf.Graph().as_default() as g:
        with tf.device("/gpu:0"):
            ...
            with tf.variable_scope("siamese") as var_scope:
                ...
                loss = loss_with_spring(x_repr1, x_repr2, y)
    ...
    return {'graph': g, 'inputs': [x1, x2, y], 'pred': y_pred, 'logits': logits, 'y_logits': y_logits,
...
def run_training(model_dict, train_data_iterator, test_full_iter,  
                 train_full_iter, n_iter=20000, print_every=500):
    testY=[]
            ...
            for iter_i, data_batch in zip(range(n_iter), train_data_iterator):
                ...
                if iter_i % print_every == 0:
                    ...
                    for data_iterator, data_name in print_zip_iter:
                        ...
                        to_compute = [model_dict['logits'], model_dict['accuracy'], model_dict['loss']]
                        logits_val, acc_value, loss_val = sess.run(to_compute, batch_feed_dict)
                        testY=np.array(test_batch[-1])
                        fmt = (iter_i, acc_value, loss_val)
                        print(data_name, 'iteration %d\t accuracy: %.3f, loss: %.3f'%fmt)
 
            tsne = TSNE(perplexity=50, n_components=2, init='pca', n_iter=5000)
            plot_only = 100
            lowDWeights = tsne.fit_transform(logits_val[0:plot_only, :])
            labels = testY[0:plot_only]
            plot_with_labels(lowDWeights, labels,'pset3_4.png')
...
mlp_args = {'hidden_sizes':[64, 64, 32, 2]}
