...
class AtariEmulator(BaseEnvironment):
    def __init__(self, rom_addr, random_start = False, random_seed = 6, visualize = True, single_life = False):
        ...
        self.observation_pool = ObservationPool(np.zeros((84, 84, 4), dtype=np.uint8))
        ...
        self.frame_pool = FramePool(np.empty((2, self.screen_height,self.screen_width), dtype=np.uint8), self.__process_frame_pool)
